# we will scrap the reviews from Flipkart
...
# Whenever we hit any url directly from browser, it goes with "GET" method
# route with allowed methods as POST and GET
@app.route('/', methods=['POST', 'GET'])
def index():
    # so, for the very first time, if url opened in browser, method will be 'GET' and the following if condition will be
    # false. Control goes to else condition where we are displaying index.html
    if request.method == 'POST':
        searchString = request.form['content'].replace(" ", "")  # obtaining the search string entered in the form
        try:
            dbConn = pymongo.MongoClient("mongodb://localhost:27017/")  # opening a connection to Mongo
            db = dbConn['crawlerDB']  # connecting to the database called crawlerDB, crawlerDB is the name we are giving, if not present, then it will be created
            ...
            if reviews.count() > 0:  # if there is a collection with searched keyword and it has records in it
                ...
                uClient = uReq(flipkart_url)  # requesting the webpage from the flipkart page
                ...
                # creating a collection with the same name as search string. Tables and
