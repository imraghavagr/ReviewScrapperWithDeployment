# Here we are removing the database part because otherwise we would need to buy database services over net
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            # table = db[searchString] # creating a collection with the same name as search string. Tables and
            # Collections are analogous. filename = searchString+".csv" #  filename to save the details fw = open(
            # filename, "w") # creating a local file to save the details headers = "Product, Customer Name, Rating,
            # Heading, Comment \n" # providing the heading of the columns fw.write(headers) # writing first the
            # headers to file
